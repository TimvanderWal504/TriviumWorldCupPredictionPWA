    ...
    ILogger<ResultIngestionJob> logger) : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        foreach (var apiFixture in allApiFixtures.Where(f => f.IsFullTime))
        {
            ...
            {
                await PurgeFixtureEventsAsync(session, tid, dbFixture.Id, ct);
            }
        ...
        foreach (var apiFixture in allApiFixtures.Where(f => f.IsLive))
        {
            ...
            if (liveEvents.Count > 0)
            {
                ...
                await PurgeFixtureEventsAsync(session, tid, liveDbFixture.Id, ct);
            ...
            foreach (var apiFixture in allApiFixtures.Where(f => f.IsLive || f.IsFullTime || f.IsCancelledOrPostponed))
            {
                ...
                if (apiFixture.IsLive)
                {
                    ...
                    if (liveSlotEvents.Count > 0)
                    {
                        ...
                        {
                            await PurgeFixtureEventsAsync(session, tid, slot.SlotKey, ct);
    ...
    /// </summary>
    private static async Task PurgeFixtureEventsAsync(IDocumentSession session, string tournamentId, string fixtureId, CancellationToken ct)
    {
        var staleGoals = await session.Query<GoalEvent>().Where(g => g.TournamentId == tournamentId && g.FixtureId == fixtureId).ToListAsync(ct);
        var staleCards = await session.Query<CardEvent>().Where(c => c.TournamentId == tournamentId && c.FixtureId == fixtureId).ToListAsync(ct);
        var staleSubs  = await session.Query<SubstitutionEvent>().Where(s => s.TournamentId == tournamentId && s.FixtureId == fixtureId).ToListAsync(ct);
        var staleVars  = await session.Query<VarEvent>().Where(v => v.TournamentId == tournamentId && v.FixtureId == fixtureId).ToListAsync(ct);
